pipeline {
  ...
  environment {
    ...
    // 源码地址
    SOURCE_URL            = 'http://192.168.157.61:31150/root/worker.git'
    // 源码凭证
    SOURCE_CERT           = 'gitlab'
    // helm地址
    HELM_URL              = 'http://192.168.157.61:31150/root/helm-charts.git'
    // helm repo
    HELM_REPO             = '192.168.157.61:31150/root/helm-charts.git'
    // docker build 的执行上下文
    DOCKER_CONTEXT        = "."
          ...
          dir("${WORKSPACE}/helm-chart") {
            ...
            withCredentials([usernamePassword(credentialsId:"${SOURCE_CERT}", passwordVariable:'password', usernameVariable:'username')]) {
              script {
                ...
                sh '''
                  yq eval '.worker.image.tag = env(IMAGE_TAG)' -i sit/app1/values.yaml
                  git config user.name "CI"
                  git config user.email "CI@DCE5.io"

                  git add sit/app1/values.yaml
                  # 检查是否有本地修改
